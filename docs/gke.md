...
This guide walks you through deploying jambonz on a GKE cluster from start to finish.
...
Or using gcloud:
```bash
gcloud compute forwarding-rules list --filter="description~traefik" --format="value(IPAddress)"
```

You can also find it in the GCP console under **Network services > Load balancing**.
